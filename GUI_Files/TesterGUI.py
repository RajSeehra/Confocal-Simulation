...
point = conf.emptysphere3D(point, radius, sphere_centre) * 10**8

xy_size = 100
pixel_size = 0.02
stack_size =100
excitation_wavelength = 0.480

laserPSF = conf.radial_PSF(xy_size, pixel_size, stack_size, excitation_wavelength)
laserPSF = np.moveaxis(laserPSF, 0, -1)     # The 1st axis was the z-values. Now in order y,x,z.

laserPSF = (laserPSF / laserPSF.sum()) * 10**8

emission_wavelength = 0.540

emissionPSF = conf.radial_PSF(xy_size, pixel_size, stack_size, emission_wavelength)
emissionPSF = np.moveaxis(emissionPSF, 0, -1)     # The 1st axis was the z-values. Now in order y,x,z.

emission_PSF = (emissionPSF / emissionPSF.sum())

scan = np.zeros((100,100,100))
laser_illum = conf.array_multiply(laserPSF,point, 30,30)
# laser_illum = conf.array_multiply(point,laserPSF, 30,30)
